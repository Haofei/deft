...
use crate::style::{FixedStyleProp, PropValueParse, StylePropKey, StylePropVal};
...
impl StyleList {
    ...
    pub fn set_style(&mut self, styles: ParsedStyles) {
        self.values = styles.0;
    }

    pub fn append_style(&mut self, styles: ParsedStyles) {
        //TODO fix mem leak(remove values with same key)
        self.values.extend(styles.0)
    }
    ...
    pub fn set_hover_style(&mut self, styles: ParsedStyles) {
    ...
    pub fn parse_style_list(str: &str) -> Vec<(&str, &str)> {
        let mut result = Vec::new();
